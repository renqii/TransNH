...
class TransEModel(object):
	...
	k = 2
        ...
        with tf.name_scope("embedding"):
            ...
            self.rel_matrix = tf.get_variable(name="rel_matrix", shape=[relation_total, size * k],
                                                  initializer=tf.contrib.layers.xavier_initializer(uniform=False))

            pos_h_e_old = tf.nn.embedding_lookup(self.ent_embeddings, self.pos_h)
            pos_t_e_old = tf.nn.embedding_lookup(self.ent_embeddings, self.pos_t)
            neg_h_e_old = tf.nn.embedding_lookup(self.ent_embeddings, self.neg_h)
            neg_t_e_old = tf.nn.embedding_lookup(self.ent_embeddings, self.neg_t)
            pos_h_e = tf.reshape(pos_h_e_old, [-1, size, 1])
            pos_t_e = tf.reshape(pos_t_e_old, [-1, size, 1])
            pos_r_e = tf.reshape(tf.nn.embedding_lookup(self.rel_embeddings, self.pos_r), [-1, size])
            neg_h_e = tf.reshape(neg_h_e_old, [-1, size, 1])
            neg_t_e = tf.reshape(neg_t_e_old, [-1, size, 1])
            neg_r_e = tf.reshape(tf.nn.embedding_lookup(self.rel_embeddings, self.neg_r), [-1, size])
            matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_matrix, self.pos_r), [-1, size,k])

            # matrix = tf.nn.l2_normalize(matrix, 1)

            pos_h_e_new = tf.matmul(tf.transpose(matrix, perm=[0, 2, 1]), pos_h_e)
            pos_h_e_new = tf.reshape(tf.matmul(matrix, pos_h_e_new), [-1, size])

            pos_t_e_new = tf.matmul(tf.transpose(matrix, perm=[0, 2, 1]), pos_t_e)
            pos_t_e_new = tf.reshape(tf.matmul(matrix, pos_t_e_new), [-1, size])

            neg_h_e_new = tf.matmul(tf.transpose(matrix, perm=[0, 2, 1]), neg_h_e)
            neg_h_e_new = tf.reshape(tf.matmul(matrix, neg_h_e_new), [-1, size])

            neg_t_e_new = tf.matmul(tf.transpose(matrix, perm=[0, 2, 1]), neg_t_e)
            neg_t_e_new = tf.reshape(tf.matmul(matrix, neg_t_e_new), [-1, size])

         
        if config.L1_flag:
            # pos = tf.reduce_sum(abs(pos_h_e + pos_r_e - pos_t_e), 1, keep_dims=True)
            # neg = tf.reduce_sum(abs(neg_h_e + neg_r_e - neg_t_e), 1, keep_dims=True)
            pos = tf.reduce_sum(abs(pos_h_e_old - pos_h_e_new + pos_r_e - (pos_t_e_old - pos_t_e_new)), 1,
                                keep_dims=True)
            neg = tf.reduce_sum(abs(neg_h_e_old - neg_h_e_new + neg_r_e - (neg_t_e_old - neg_t_e_new)), 1,
                                keep_dims=True)
            self.predict = pos
        else:
            # pos = tf.reduce_sum((pos_h_e + pos_r_e - pos_t_e) ** 2, 1, keep_dims=True)
            # neg = tf.reduce_sum((neg_h_e + neg_r_e - neg_t_e) ** 2, 1, keep_dims=True)
            pos = tf.reduce_sum((pos_h_e_old - pos_h_e_new + pos_r_e - (pos_t_e_old - pos_t_e_new)) ** 2, 1,
                                keep_dims=True)
            neg = tf.reduce_sum((neg_h_e_old - neg_h_e_new + neg_r_e - (neg_t_e_old - neg_t_e_new)) ** 2, 1,
                                keep_dims=True)
            self.predict = pos
